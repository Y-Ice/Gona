...
const languages = [
  { code: "NG", label: "English" },
  { code: "NG", label: "Hausa" },
  { code: "NG", label: "Yorùbá" },
  { code: "NG", label: "Igbo" },
];
      ...
      <div className="px-6 py-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          {languages.map((lang) => (
            ...
            >
              <span className={`text-lg font-bold ${selected === lang.label ? "text-[#c47a0a]" : "text-gray-700"}`}>
                {lang.code}
              </span>
              <span className={`text-sm font-sans ${selected === lang.label ? "text-[#c47a0a] font-medium" : "text-gray-500"}`}>
                {lang.label}
              </span>
            </button>
          ))}
        ...
        <div className="flex items-start gap-2 bg-[#f0ece0] rounded-xl px-4 py-3">
          ...
          <p className="text-sm text-gray-600 font-sans">
            Currently set to: <span className="font-semibold text-gray-800">{selected}</span>. Navigation labels will update immediately.
          </p>
        </div>
      </div>
    </div>
