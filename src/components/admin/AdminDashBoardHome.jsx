...
const AdminDashBoardHome = () => {
  ...
  const getFarmName = (id) =>
    farms.find((f) => f._id === id)?.name || "Unknown Farm";
  const getEmpName = (id) =>
      ...
      <div className="px-4 sm:px-6">
            ...
            <div className="flex flex-col gap-4">
              {recentActivities.map((act) => (
                <div key={act._id} className="flex items-start gap-4">
                  <span
                    className={`text-xs font-sans font-semibold px-3 py-1.5 rounded-full flex-shrink-0 ${activityTypeColor(act.type)}`}
                  >
                    <T text={act.type} />
                  </span>
                  <div className="flex-1">
                    ...
                    <p className="text-[11px] text-gray-400 font-sans mt-0.5 tracking-wide">
                      {getFarmName(act.farmId)} · {getEmpName(act.employeeId)} ·{" "}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-2 px-6 py-3 bg-[#f7f4ee] border-y border-gray-100">
            {["Farm Name", "Location", "Specialization", "Size", "Status"].map(
              (col) => (
                <span
                  key={col}
                  className="text-xs font-sans font-semibold text-gray-400"
                >
                  <T text={col} />
                </span>
              ),
